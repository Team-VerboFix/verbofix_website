...
def predict_stammer(features):
    if len(features) == 0:
        return []
    # ensure features are float32 and correct shape for the model
    features = np.asarray(features, dtype=np.float32)
    predictions = model.predict(features, verbose=0)  # shape (n, 1) or (n,)
    predictions = np.asarray(predictions).reshape(-1)  # flatten safely
    preds_bin = (predictions > THRESHOLD).astype(int)
    confidences = predictions
    # convert numpy types to pure python types here
    return [(int(p), float(c)) for p, c in zip(preds_bin, confidences)]
...
def group_consecutive_chunks(predictions, audio_duration, hop_time):
    ...
    ranges = []
    current_label = int(predictions[0][0])
    start_idx = 0
    current_confidences = [float(predictions[0][1])]
    chunk_indices = [0]

    for i in range(1, len(predictions)):
        label_i = int(predictions[i][0])
        conf_i = float(predictions[i][1])
        if label_i == current_label:
            ...
            avg_confidence = float(np.mean(current_confidences)) if current_confidences else 0.0
            label_str = "Stammered" if current_label == 1 else "Fluent"
            ranges.append((float(start_time), float(end_time), label_str, avg_confidence, list(chunk_indices)))
            current_label = label_i
            start_idx = i
            current_confidences = [conf_i]
            chunk_indices = [i]
...
def generate_report(ranges, total_chunks, audio_duration, transcription):
    ...
    for start_time, end_time, label, avg_conf, chunk_indices in ranges:
        num_chunks_in_range = int(len(chunk_indices))
        if label == "Stammered":
    ...
    stammer_rate = (stammered_chunks / total_chunks) * 100 if total_chunks > 0 else 0.0
    ...
    # Ensure all numbers are plain python types (int/float/str/list)
    return {
        ...
        "audioDuration": f"{float(audio_duration):.1f}s",
        "severity": severity,
